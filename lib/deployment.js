...
var checkAndDeploy = function(checkAndDeployCb) {
  ...
  var checkDatabases = function(checkDatabasesCb) {
    ...
    myutils.doParallel([
      function(cb) {checkDatabase(config.eventsDb, './designdocs/lively_events', '_design/lively_events', cb)},
      function(cb) {checkDatabase(config.workersDb, null, null, cb)},
      function(cb) {checkDatabase(config.logsDb, './designdocs/lively_logs', '_design/lively_logs', cb)}
    ], checkDatabasesCb);
  };
  
    workerPath = config.workersDeployed;
    workerSourcePaths = config.workersSource;
    eventSourcePaths = config.eventsSource;
    logEvents = config.logToCouch;
...
exports.createWorkerChangeListener = createWorkerChangeListener;
exports.createSourceChangeListener = createSourceChangeListener;
exports.writeOutWorkerCode = writeOutWorkerCode;
exports.workerPath = workerPath;
exports.logEvents = logEvents;