...
var handleHTTPEvent = function(eventName, eventParameters, response) {
  ...
  if(eventParameters.postData)
    event.postData = eventParameters.postData;
...
var updateEventDefinitions = function(cb) {
  config.eventsDb.view('lively_events', 'triggering-urls', {}, function(err, resp) {
    var rows = resp.rows;
    eventDefinitions = resp.rows.map( function(each) {
      return each.value
    });
...
exports.createEventsChangeListener = createEventsChangeListener;